...
struct SearchResult {
    ...
    SearchResult(const std::string &boardName, const std::string &algorithm, const std::string &heuristic = "",
        ...
        : boardFileName(boardName), algorithm(algorithm), heuristic(heuristic), opponent(opponent),
          opponentHeuristic(opponentHeuristic), moves(0), executionTime(0), expandedNodes(0),
    ...
    void writeToCSV(const std::string &filename) const {
        ...
        file << boardName << "," << algorithm << "," << heuristic << "," << opponent << "," << opponentHeuristic << ","
             << moves << "," << static_cast<double>(executionTime) << "," << expandedNodes << ",";
    ...
    void writeSolutionToFile(const std::string &directory) const {
        ...
        for (size_t i = 0; i < solution.size(); ++i) {
            ...
            std::string dirStr =
                (move.direction == MoveDirection::FORTH) ? carDirMap[move.carId].first : carDirMap[move.carId].second;
