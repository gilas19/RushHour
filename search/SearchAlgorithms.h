...
struct BoardState {
    ...
    bool operator==(const BoardState& other) const {
        return board.hash() == other.board.hash();
    }
};
...
struct MCTSNode {
    ...
    bool isFullyExpanded() const {
        return children.size() == board.generatePossibleMoves(player).size();
    }

    bool isTerminal() const {
        return board.isWon(Player::HORIZONTAL) || board.isWon(Player::VERTICAL);
    }
};
...
namespace SearchAlgorithms {
    ...
    std::vector<Move> AStar(Board& board, HeuristicFunc heuristic, SearchResult& result, Player player = Player::HORIZONTAL);   // A* Search
    ...
    std::vector<Move> AdverserialGame(Board& board, AdverserialPlayers players, SearchResult& result);                          // Full Game Alpha-Beta Pruning
    Move MCTS(Board& board, Player player, int iterations = 100, double explorationParameter = 1.2);                           // Monte Carlo Tree Search
}
