...
class PgParser:
    ...
    def parse_actions_and_propositions(self):
        ...
        words = [
            word.rstrip()
            for word in proposition_line.split(" ")
        ...
        while line != "":
            words = [
                word.rstrip() for word in line.split(" ") if len(word.rstrip()) > 0
            ]
            if words[0] == "Name:":
                name = words[1]
                line = f.readline()
        ...
        for a in actions:
            ...
            new_delete = [
                p for p in propositions if p.name in [q.name for q in a.delete]
            ]
            a.pre = new_pre
            a.add = new_add
            a.delete = new_delete
